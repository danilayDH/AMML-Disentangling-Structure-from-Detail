...
class ResNetEncoder(nn.Module):
    def __init__(self, block, layers, first_conv=False, maxpool1=False, num_channels=1) -> None:
        super().__init__()
...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self, block, layers, latent_dim, input_height, first_conv=False, maxpool1=False,
                 use_segmentation_masks="no_mask") -> None:
        super().__init__()
        ...
        if use_segmentation_masks == "in_decoder":
            self.linear = nn.Linear(latent_dim + 256, self.inplanes * 4 * 4)
        ...
        # Layers for segmentation mask
        self.conv_mask1 = nn.Conv2d(32, 64, kernel_size=3, padding=1)
        self.conv_mask2 = nn.Conv2d(64, 128, kernel_size=3, padding=1)
        self.conv_mask3 = nn.Conv2d(128, 256, kernel_size=3, padding=1)
        self.global_pool = nn.AdaptiveAvgPool2d(1)

    def _make_layer(self, block, planes, blocks, scale=1):
        upsample = None
        if scale != 1 or self.inplanes != planes * block.expansion:
            upsample = nn.Sequential(resize_conv1x1(self.inplanes, planes * block.expansion, scale),
                nn.BatchNorm2d(planes * block.expansion), )
    ...
    def forward(self, x):
        if self.use_segmentation_masks == "in_decoder":
            masks = x[1]
            masks = F.relu(self.conv_mask1(masks))
            masks = F.relu(self.conv_mask2(masks))
            masks = F.relu(self.conv_mask3(masks))
            masks = self.global_pool(masks)
            masks = torch.flatten(masks, 1)
            x = torch.cat((x[0], masks), dim=1)
        else:
            x = x[0]
...
def resnet18_encoder(first_conv, maxpool1, num_channels):
    return ResNetEncoder(EncoderBlock, [2, 2, 2, 2], first_conv, maxpool1, num_channels)


def resnet18_decoder(latent_dim, input_height, first_conv, maxpool1, use_segmentation_masks):
    return ResNetDecoder(DecoderBlock, [2, 2, 2, 2], latent_dim, input_height, first_conv, maxpool1,
                         use_segmentation_masks)


def resnet50_encoder(first_conv, maxpool1, num_channels):
    return ResNetEncoder(EncoderBottleneck, [3, 4, 6, 3], first_conv, maxpool1, num_channels)


def resnet50_decoder(latent_dim, input_height, first_conv, maxpool1, use_segmentation_masks):
